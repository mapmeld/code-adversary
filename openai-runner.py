...
def explain_code(code, code_type="class"):
    if code_type == "query":
        code_prompt = f"-- Explanation of the above {code_type} in human readable format\n--\n1."
    else:
        code_prompt = f"\n\"\"\"\nHere's what the above {code_type} is doing:\n1."
    continue_code(code + code_prompt, start_output="1.")
    ...
    if os.path.isdir(os.path.join('code-explain', subdir)):
        for pypath in os.listdir(os.path.join('code-explain', subdir)):
            if len(runfiles) == 0 or pypath in runfiles:
                if '.py' in pypath or '.sql' in pypath:
                    print(pypath + "\n###\n")
